...
const PMTILES_PATH = "tract-d/data/pmtiles";
...
async function main(): Promise<void> {
  ...
  const tilesets = (await fs.readdir(path.resolve(__dirname, "../data/output")))
  ...
  for (const tileset of tilesets) {
    ...
    const file = await fs.readFile(
      path.resolve(__dirname, `../data/output/${tileset}.pmtiles`),
    );
